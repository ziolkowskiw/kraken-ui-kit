...
const cases = [
  ...
  { id: 'components-contextmenu--playground', act: (p) => txt(p, 'Right-click here').click({ button: 'right' }) },
  ...
  { id: 'components-combobox--playground', act: async (p) => { const i = p.locator('input').first(); await i.click(); await i.pressSequentially('a', { delay: 40 }) } },
  { id: 'components-command--playground', act: async (p) => { const i = p.locator('input').first(); await i.click(); await i.pressSequentially('ca', { delay: 40 }) } },
  { id: 'components-select--playground', act: async (p) => { await p.locator('[data-slot="select-trigger"]').first().click() } },
  { id: 'components-dialog--playground', act: async (p) => { await p.locator('[data-slot="dialog-trigger"]').first().click() } },
  { id: 'components-carousel--playground', act: async (p) => { await p.locator('[data-slot="carousel-next"]').first().click() } },
  { id: 'components-tooltip--tooltip-icon-playground', act: async (p) => { await p.locator('[data-slot="tooltip-trigger"]').first().hover() } },
]
...
for (const c of cases) {
  ...
  const errors = []
  page.on('console', (m) => { if (m.type() === 'error') { const t = m.text(); if (!IGNORE.some((re) => re.test(t))) errors.push('console: ' + t.replace(/\s+/g, ' ').slice(0, 280)) } })
  page.on('pageerror', (e) => errors.push('pageerror: ' + (e.message || String(e)).replace(/\s+/g, ' ').slice(0, 280)))
  try {
    await page.goto(`${BASE}/iframe.html?viewMode=story&id=${c.id}`, { waitUntil: 'load', timeout: 20000 })
    ...
    errors.push('interaction: ' + String(e.message || e).replace(/\s+/g, ' ').slice(0, 200))
  }
...
for (const r of bad) { console.log(`■ ${r.id}`); r.errors.slice(0, 4).forEach((e) => console.log('   - ' + e)) }
