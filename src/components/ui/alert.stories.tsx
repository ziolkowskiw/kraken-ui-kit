...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          "displays a callout for user attention; inline, non-blocking messages (info, success, warning, error)",
      },
    },
  },
  tags: ["autodocs"],
  ...
  render: ({
    ...
    return (
      ...
      >
        ...
        {hasDescription && <AlertDescription>{descriptionText}</AlertDescription>}
      </Alert>
    );
  },
    ...
    <div className="flex flex-col gap-4 max-w-md">
      {TYPE_OPTIONS.map((t) => (
        <Alert key={t} type={t} icon={iconMap[t]} closeIcon={<X />} onClose={() => {}}>
          ...
          <AlertDescription>This is a {t} alert with icon and description.</AlertDescription>
        </Alert>
      ))}
    </div>
  ),
