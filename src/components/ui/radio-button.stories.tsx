...
const meta = {
  ...
  render: ({ hasTooltip, tooltipText, ...args }: StoryProps) => (
    <div className="w-[480px]">
      <RadioButtonGroup {...args} tooltip={hasTooltip ? tooltipText : undefined} defaultValue="standard">
        <RadioButton value="standard" size={args.size} label="Standard" secondLineLabel="3–5 days" />
        <RadioButton value="express" size={args.size} label="Express" secondLineLabel="1–2 days" />
        <RadioButton value="overnight" size={args.size} label="Overnight" secondLineLabel="Next day" />
      </RadioButtonGroup>
    </div>
  ),
