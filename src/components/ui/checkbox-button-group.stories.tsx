...
const meta: Meta<GroupStoryProps> = {
  ...
  argTypes: {
    ...
    secondLineLabel: { control: 'text', name: 'checkbox-button / secondLineLabel', if: { arg: 'hasSecondLine' } },
    optionCount: { control: { type: 'range', min: 1, max: MAX_OPTIONS, step: 1 }, name: 'Number of options', table: { category: 'Options' } },
  ...
  render: ({
    state, errorMessage, size, cbVariant, hasSecondLine, secondLineLabel,
    hasTooltip, tooltipText, optionCount,
    option1Label, option2Label, option3Label, option4Label, option5Label, option6Label,
    ...args
  }) => {
    const isError = state === 'error'
    const allLabels = [option1Label, option2Label, option3Label, option4Label, option5Label, option6Label]
